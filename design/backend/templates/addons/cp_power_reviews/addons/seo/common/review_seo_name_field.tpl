{if !$hide_title}
{include file="common/subheader.tpl" title=__("seo") target="#review_acc_addon_seo"}
{/if}
<div id="review_acc_addon_seo" class="collapsed in">
    <div class="control-group {if $share_dont_hide}cm-no-hide-input{/if}">
        ...
        <div class="controls cm-seo-check-changed-block-container">
            ...
            <div class="input-prepend input-prepend--mobile-fullwidth">
                <span class="cm-field-prefix add-on">{$parent_uri.prefix}</span>
                <input type="text"
                    name="{$object_name}[seo_name]"
                    id="elm_seo_name"
                    size="10"
                    value="{$object_data.seo_name}"
                    class="input-long cm-seo-check-changed"
                /><span class="cm-field-suffix">{$parent_uri.suffix}</span>
                {if $preview_link}
                    <a class="btn cp-pre__preview-page-link" href="{$preview_link}" target="_blank">{__("preview")}</a>
                {/if}
            </div>

            <div class="hidden cm-seo-check-changed-block">
                <input type="hidden"
                    name="{$object_name}[seo_create_redirect]"
                    disabled="disabled"
                    value="0"
                />
