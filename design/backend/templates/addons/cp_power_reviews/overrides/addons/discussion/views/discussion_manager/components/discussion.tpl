                ...
                {if "discussion.add"|fn_check_view_permissions && !("MULTIVENDOR"|fn_allowed_for && $runtime.company_id && ($runtime.company_id != $object_company_id || $discussion.object_type == 'M'))}
                    {if $discussion.object_type == "E"}
                        {capture name="adv_buttons"}
                            {include file="common/popupbox.tpl" id="add_new_post" title=__("cp_pr_add_review") icon="icon-plus" act="general" link_class="cm-dialog-switch-avail"}
                        {/capture}
                    {else}
                        {include file="common/popupbox.tpl" id="add_new_post" link_text=__("cp_pr_add_review") act="general" link_class="cm-dialog-switch-avail"}
                    {/if}
                {/if}
