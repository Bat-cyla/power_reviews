    ...
    {if $runtime.company_id || !"MULTIVENDOR"|fn_allowed_for || ("MULTIVENDOR"|fn_allowed_for && !$runtime.company_id)}
        {if $attributes}
            <form action="{""|fn_url}" method="post" name="apply_attributes" class="form-horizontal form-edit">
                ...
                <div id="apply_attribute_to_categories">
                    <fieldset>
                        {include file="pickers/categories/picker.tpl" hide_input=true input_name="applyed_to_categories" data_id="added_categories" but_meta="pull-right" multiple=true}
                    </fieldset>
                </div>

                {capture name="buttons"}
                    {include file="buttons/save.tpl" but_name="dispatch[cp_pow_rev.apply_attr_to_cat]" but_role="submit-link" but_target_form="apply_attributes"}
                {/capture}
            </form>
        {else}
