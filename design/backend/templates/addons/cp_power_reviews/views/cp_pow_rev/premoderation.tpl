{$title_page = __("cp_power_reviews.reply_moder")}
{if $auth.user_type === "UserTypes::VENDOR"|enum}
    {$title_page = __("cp_power_reviews.receil_reply_moder")}
{/if}
...
{include file="common/mainbox.tpl" title=$title_page content=$smarty.capture.mainbox title_extra=$smarty.capture.title_extra sidebar=$smarty.capture.sidebar buttons=$smarty.capture.buttons adv_buttons=$smarty.capture.adv_buttons}


{if $addons.cp_power_reviews.allow_reply_rev == "Y" && $discussion.posts && $auth.user_type === "UserTypes::VENDOR"|enum}
    {foreach $discussion.posts as $post}
        {$form_content_capture = "content_for_popup_reply_`$post.post_id`"}
        {include file="common/popupbox.tpl" id="reply_post_`$post.post_id`" text=__("cp_pr_add_review") content=$smarty.capture.$form_content_capture act="fake"}
    {/foreach}
{/if}