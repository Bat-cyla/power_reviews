...
<div id="box_attach_images_{$name}_{$key}" class="cp-attach-images">
    ...
    <div class="cp-upload-box {if $no_thumbnail && !$pair.icon}hidden{/if}" id="load_thumbnail_{$name}{$suffix}{$key}">
    ...
    {/if}
        ...
        <div class="cp-pull-left cp-image-wrap">
            ...
            {if !$hide_images}
                <div class="cp-image">
                    {if $pair.image_id}
                        {include file="addons/cp_power_reviews/components/image.tpl" image=$pair.icon image_id=$pair.image_id image_width=150}
                    {else}
        ...
        <div class="cp-image-upload cm-hide-with-inputs">
            {include file="addons/cp_power_reviews/components/fileuploader.tpl" var_name="`$name`_image_icon`$suffix``$more_index`[`$key`]" is_image=true }
        </div>
    </div>
    {if !$no_detailed}
        {if $detailed_title}
        ...
        <div class="cp-upload-box">
            <div class="cp-pull-left cp-image-wrap">
                {if !$hide_images}
                    ...
                    <div class="cp-image">
                        {if $pair.detailed_id}
                            {include file="addons/cp_power_reviews/components/components/image.tpl" image=$pair.detailed image_id=$pair.detailed_id image_width=150}
                        {else}
                            <div class="cp-no-image">
                                {if $cp_image_icon}
                                    <img src="{$cp_image_icon}" width="150px" border="0" alt="" />
            ...
            <div class="cp-image-upload cm-hide-with-inputs">
                {include file="addons/cp_power_reviews/components/fileuploader.tpl" var_name="`$name`_image_detailed`$suffix``$more_index`[`$key`]" }
            </div>
        </div>
    {/if}
</div>
