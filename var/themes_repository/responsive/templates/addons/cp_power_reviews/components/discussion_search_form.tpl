...
<form action="{""|fn_url}" name="discussion_search_form" method="get" class="cm-disable-empty {$form_meta}">

{if $put_request_vars}
{foreach from=$smarty.request key="k" item="v"}
{if $v && $k != "callback" && ($k == "dispatch" || $k == "checkbox_name" || $k == "data_id" || $k == "result_ids" || $k == "skip_result_ids_check" || $k == "is_ajax" || $k == "is_search")}
...
<div class="group form-horizontal">
    <div class="control-group">
      <label class="control-label" for="discussion_type">{__("discussion")}</label>
      <div class="controls">    
...
{include file="common/advanced_search.tpl" simple_search=$smarty.capture.simple_search advanced_search=$smarty.capture.advanced_search dispatch=$dispatch view_type="discussion"}
