...
<input type="hidden" {if $label_id}id="{$label_id}"{/if} value="{if $images}{$id_var_name}{/if}" />
...
<div id="file_uploader_{$id_var_name}">
    ...
    {strip}
        ...
        <div class="cp-btn-group {if $multiupload != "Y" && $images}hidden{/if}" id="link_container_{$id_var_name}">
            <div class="cp-upload-file-local">
                <a class="cp-btn">
                    <span data-ca-multi="Y" {if !$images}class="hidden"{/if}>
                        {$upload_another_file_text|default:__("upload_another_file")}
                    </span>
                    <span data-ca-multi="N" >
                        {$upload_file_text|default:__("local")}
                    </span>
                </a>
                <div class="cp-image-selector">
                    <label for="">
            ...
            <a class="cp-btn cp-url-btn" onclick="Tygh.fileuploader.show_loader(this.id); Tygh.fileuploader.check_required_field('{$id_var_name}', '{$label_id}');" id="url_{$id_var_name}">{__("url")}</a>
            {if $hidden_name}
