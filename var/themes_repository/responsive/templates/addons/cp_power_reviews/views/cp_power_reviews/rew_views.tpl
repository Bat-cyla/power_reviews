{if $smarty.const.AJAX_REQUEST && $product.discussion}
    {$discussion=$product.discussion}
{else}
{if $object_type == "P" && $smarty.const.CP_PR_VARIATIONS_TYPE && $smarty.const.CP_PR_VARIATIONS_TYPE == "Y" && $product.variation_group_id}
    {$rev_params=["req" => $smarty.request, "from_prod_tab" => 1]}
    {$discussion=$object_id|fn_get_discussion:$object_type:false:$rev_params}
    {*
    {$back_data=$rev_params|fn_cp_power_reviews_get_power_reviews}
    {if $back_data[1][cur_disc_type] != "D"}
        {$discussion=["type" => $back_data[1].cur_disc_type, "posts" => $back_data[0]]}
    {/if}
    *}
{else}
    {if $back_disc}
...
{if $discussion && $discussion.type != "D"}
    ...
    {/if}
    {$cur_result_id="pagination_contents_comments_`$object_id`,cp_posts_list_`$object_id`"}
